...
h1's response
    ...
    public LifecycleEvent awaitNthResponse(int n) throws InterruptedException {
        ...
        if (latches[n].await(30, TimeUnit.SECONDS)) {//wait for ackcheck response before hitting ack endpoint  
            return this.latches[n].getLifecycleEvent();
        } else {
            LOG.warn("ResponseCoordinator timed out waiting 30 sec for response {}.", n);
            return null;
        }
    }
