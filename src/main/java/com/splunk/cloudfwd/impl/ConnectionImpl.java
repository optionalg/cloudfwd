...
/**
 * Internal implementation of Connection. Should be obtained through a Factory
 * method.
 *
 * @author ghendrey
 */
...
//  }
    ...
    /**
     * The send method will send the Event immediately unless buffering is
     * enabled. Buffering is enabled via either the setEventBatchSize method, or
     * the EVENT_BATCH_SIZE property key. The buffer is flushed either by
     * closing the Connection, calling flush, or calling send until
     * EVENT_BATCH_SIZE bytes have accumulated in the Connections internal
     * EventBatchImpl. When an EventBatchImpl is flushed, the connection's
     * ConnectionCallbacks will be invoked, asynchronusly. The send method may
     * block for up to BLOCKING_TIMEOUT_MS milliseconds before throwing an
     * HecConnecionTimeoutException.
     *
     * @param event
     * @return the number of bytes sent (will be zero unless buffer reaches
     * EVENT_BATCH_SIZE and flushes)
    ...
    public synchronized int sendBatch(EventBatch events) throws HecConnectionTimeoutException, HecNoValidChannelsException {
        if (closed) {
            throw new HecConnectionStateException(
                    "Attempt to sendBatch on closed connection.",
                    HecConnectionStateException.Type.SEND_ON_CLOSED_CONNECTION);
        }
        ...
        ((EventBatchImpl) events).checkAndSetCompatibility(propertiesFileHelper.
                getHecEndpointType());
        timeoutChecker.start();
        timeoutChecker.add((EventBatchImpl) events);
        LOG.debug("sending  characters {} for id {}", events.getLength(),
                events.getId());
    ...
    private void throwExceptionIfNoChannelOK() {
        ...
        if (healths.isEmpty()) {
            throw new HecConnectionStateException(
                    "No HEC channels could be instatiated on Connection.",
